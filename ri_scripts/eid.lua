...
local ItemDescriptionsEnglish = {
    ...
    [CollectibleType.COLLECTIBLE_BEAN] = "{{Poison}} Farts a poison cloud when the player uses an active item#The poison deals Isaac's damage 6 times#The size of the fart scales with active charge",
    ...
    [CollectibleType.COLLECTIBLE_IPECAC] = "↑ {{Damage}} +4 Damage#↑ {{Damage}} x2 Damage multiplier#↓ {{Tears}} x0.33 Fire rate multiplier#↓ {{Range}} x0.8 Range multiplier#↓ {{Shotspeed}} x0.8 Shot speed multiplier#Isaac's tears are fired in an arc#{{Poison}} The tears explode and poison enemies where they land",
    ...
    [CollectibleType.COLLECTIBLE_BURSTING_SACK] = "Spider enemies no longer target or deal contact damage to Isaac#{{Slow}} Slow immunity",
    [CollectibleType.COLLECTIBLE_GB_BUG] = "{{Throwable}} Throwable (double-tap shoot)#Devolves enemies and rerolls pickups it comes in contact with",
    ...
    --[CollectibleType.COLLECTIBLE_CLICKER] = "Changes the current room layout#Can be used to get multiple room clear rewards from a single room#Cannot be used to change special rooms",
    [CollectibleType.COLLECTIBLE_DUALITY] = "{{AngelDevilChance}} Spawns an alternating door to a Devil or Angel room if either would have spawned#Entering one makes the other one unenterable",
    [CollectibleType.COLLECTIBLE_MOMS_BRACELET] = "Allows Isaac to pick up and throw rocks, ToNT, poops, slots, friendly Dips, Hosts and other obstacles#Allows carrying them between rooms",
    [CollectibleType.COLLECTIBLE_MEGA_MUSH] = "Gigantifies Isaac and grants:#↑ {{Damage}} x4 Damage multiplier#↑ {{Range}} +2 Range#↓ {{Tears}} -1.9 Tears#Invincibility#Ability to crush enemies and obstacles#{{Timer}} Lasts for 30 seconds and persists between rooms and floors#Items do not recharge whilst active",
...
local CardDescriptionsEnglish = {
    ...
    [Card.RUNE_BERKANO] = "Summons 4-8 blue flies and blue spiders",
...
if FiendFolio then
    CardDescriptionsEnglish[Card.RUNE_BERKANO] = "Summons 4-8 blue flies, blue spiders, and blue scuzz"
end
